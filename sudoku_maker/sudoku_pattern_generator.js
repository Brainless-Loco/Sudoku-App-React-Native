...
const generate_a_new_pattern = ()=>{
  let got_pattern=false;

  let SUDOKU = make_array(9,9), row = make_array(9,10),
    col = make_array(9,10), matrix = make_array(10,10);

  
  const create_pattern = (x,y,SUDOKU,row,col,matrix) =>
  {
    ...
    while(till_now.size<9 && !got_pattern){
      ...
      if(!row[x][new_num] && !col[y][new_num] && !matrix[mat_num][new_num]){
        ...
        else create_pattern(new_x,new_y,SUDOKU,row,col,matrix);
